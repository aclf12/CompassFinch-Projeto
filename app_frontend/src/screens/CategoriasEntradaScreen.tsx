...
export default function CategoriasEntradaScreen({ navigation }: any) {
  ...
  async function carregarCategorias() {
    try {
      ...
      // FILTRA APENAS CATEGORIAS DE ENTRADA (RECEITAS) DIRETO PELO TIPO DO BANCO
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Categorias de Entrada</Text>
        <View style={{ width: 28 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer} showsVerticalScrollIndicator={false}>
        {categorias.length === 0 ? (
          <Text style={styles.emptyText}>Nenhuma categoria de entrada mapeada.</Text>
        ) : (
          categorias.map((item) => {
            ...
            const totalRecebidoNoMes = historicoFiltrado.reduce((acc: number, t: any) => acc + Number(t.valor || 0), 0);
              ...
              <View key={item.id} style={styles.cardContainer}>
                ...
                >
                  ...
                  <View style={styles.categoryRight}>
                    <Text style={styles.categoryValue}>{formatarParaMoeda(totalRecebidoNoMes)}</Text>
...
const styles = StyleSheet.create({
  ...
  // COR MUDADA AQUI NAS ENTRADAS
  historyValue: { color: '#00FF4D', fontSize: 11, fontWeight: 'bold' },
